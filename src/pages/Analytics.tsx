...
import { BarChart2, PieChart, TrendingUp, Filter, ChevronDown } from 'lucide-react';
...
const Analytics = () => {
  ...
  const calculateStats = () => {
    ...
    const avgConfidence = surveys.reduce((sum, s) => sum + s.confianca, 0) / totalSurveys;
    const avgInterest = surveys.reduce((sum, s) => sum + s.interesse, 0) / totalSurveys;
  ...
  const prepareChartData = () => {
    ...
    const votingIntention = {
      'Sim': surveys.filter(s => s.vai_votar === 'Sim').length,
      'Não': surveys.filter(s => s.vai_votar === 'Não').length,
    };
    ...
    <div className="space-y-6">
        ...
        <Card>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <select
                className="input w-full"
                value={filters.ageRange ? `${filters.ageRange[0]}-${filters.ageRange[1]}` : ''}
                onChange={(e) => {
        ...
        <Card>
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-xl font-semibold">{stats?.totalSurveys || 0}</p>
            </div>
          </div>
        </Card>
        ...
        <Card>
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-xl font-semibold">{stats?.avgConfidence || 0}/10</p>
            </div>
          </div>
        </Card>
        ...
        <Card>
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-xl font-semibold">{stats?.avgInterest || 0}/10</p>
            </div>
          </div>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <ChartContainer
          ...
          type="bar"
          data={chartData?.satisfactionDistribution || { labels: [], datasets: [] }}
        />
      </div>
